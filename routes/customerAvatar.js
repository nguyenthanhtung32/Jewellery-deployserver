...
// http://127.0.0.1:9000/upload/customers/:id/avatar
router.post("/:collectionName/:id/avatar", async (req, res, next) => {
  ...
  upload(req, res, async (err) => {
    if (err instanceof multer.MulterError) {
      ...
      updateDocument(
        id,
        { avatarUrl: `/uploads/${collectionName}/${id}/${req.file.filename}` },
        collectionName
      );
      //
